...
<body>
	...
	function replace_image(index){
		//获得图片对象
		var blob_image = $("#file_"+index)[0].files[0];//浏览器缓存里的blob_image
		var url = window.URL.createObjectURL(blob_image);//获得浏览器中url的对象，该对象指向blob的图片对象（即blob_image）
	...
	function add_image(index){
		...
		var c = '<img id="image_'+(index+1)+'" onclick="click_image('+(index+1)+')" style="cursor: pointer;" src="image/upload_hover.png" width="80px" height="80px">';
		var d = '</div>';
